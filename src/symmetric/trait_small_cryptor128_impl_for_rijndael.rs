...
impl <const ROUND: usize, const IRREDUCIBLE: u8, const AFFINE_MUL: u64,
...
SmallCryptor<u128, 16> for Rijndael_Generic<ROUND, 4, 4, IRREDUCIBLE, AFFINE_MUL, AFFINE_ADD, SR0, SR1, SR2, SR3,
        ...
        RC0, RC1, RC2, RC3, RC4, RC5, RC6, RC7, RC8, RC9, ROT>
{
    ...
    fn encrypt_unit(&mut self, message: u128) -> u128
    {
        ...
        let m = [IntUnion::new_with(longer.get_uint_(0)),
                                IntUnion::new_with(longer.get_uint_(1)),
                                IntUnion::new_with(longer.get_uint_(2)),
                                IntUnion::new_with(longer.get_uint_(3)),];
        let res = self._encrypt(&m);
        LongerUnion::new_with_uints([res[0].get(), res[1].get(), res[2].get(), res[3].get()]).get()
    }
    
    #[inline] fn decrypt_unit(&mut self, cipher: u128) -> u128
    {
        let longer = LongerUnion::new_with(cipher);
        let c = [IntUnion::new_with(longer.get_uint_(0)),
                                IntUnion::new_with(longer.get_uint_(1)),
                                IntUnion::new_with(longer.get_uint_(2)),
                                IntUnion::new_with(longer.get_uint_(3))];
