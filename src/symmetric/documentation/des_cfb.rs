...
>
{
    ...
    pub fn decrypt_with_padding_pkcs7_pcbc(&mut self, cipher: *const u8, length_in_bytes: u64, iv: u64, message: *mut u8) -> u64
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_cfb_into_vec<T>(&mut self, cipher: *const u8, length_in_bytes: u64, iv: u64, message: &mut Vec<T>) -> u64
    where T: SmallUInt + Copy + Clone
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_cfb_into_array<T, const N: usize>(&mut self, cipher: *const u8, length_in_bytes: u64, iv: u64, message: &mut [T; N]) -> u64
    where T: SmallUInt + Copy + Clone
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_cfb_into_string(&mut self, cipher: *const u8, length_in_bytes: u64, iv: u64, message: &mut String) -> u64
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_vec_with_padding_pkcs7_pcbc<T>(&mut self, cipher: &Vec<T>, iv: u64, message: *mut u8) -> u64
    where T: SmallUInt + Copy + Clone
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_vec_cfb_into_vec<T, U>(&mut self, cipher: &Vec<T>, iv: u64, message: &mut Vec<U>) -> u64
    where T: SmallUInt + Copy + Clone, U: SmallUInt + Copy + Clone
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_vec_cfb_into_array<T, U, const N: usize>(&mut self, cipher: &Vec<T>, iv: u64, message: &mut [U; N]) -> u64
    where T: SmallUInt + Copy + Clone, U: SmallUInt + Copy + Clone
    {
        unimplemented!(); // Dummy code for documentation
    }

    pub fn decrypt_vec_cfb_into_string<T>(&mut self, cipher: &Vec<T>, iv: u64, message: &mut String) -> u64
    where T: SmallUInt + Copy + Clone
    {
        unimplemented!(); // Dummy code for documentation
    }
