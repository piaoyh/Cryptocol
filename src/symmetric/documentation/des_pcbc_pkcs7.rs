...
impl <const ROUND: usize> DES_Generic<ROUND>
{
    ...
    /// use std::io::Write;
    /// use std::fmt::Write as _;
    /// use cryptocol::symmetric::{ DES_Expanded, PCBC_PKCS7 };
    /// 
    /// let key = 0x_1234567890ABCDEF_u64;
    /// println!("K =\t{:#016X}", key);
    ...
    /// Encrypts the data stored in a String object with the padding according
    /// to PKCS #7 in PCBC (Propagation Cipher-Block Chaining) mode, and stores the encrypted
    /// data in `Vec<U>`.
    /// 
    /// # Arguments
    /// - `iv` is an initial value for CBC mode.
    ...
    /// to PKCS #7 in PCBC (Propagation Cipher-Block Chaining) mode, and stores the decrypted
    /// data in `Vec<V>`.
    /// 
    /// # Arguments
    /// - `iv` is an initial value for CBC mode.
