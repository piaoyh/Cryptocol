...
where T: TraitsBigUInt<T>,
    ...
    where U: TraitsBigUInt<U>
    {
        biguint_general_modular_calc_pow_assign!(self, U::u8_as_smalluint(1), exp, modulo);
    }
    ...
    pub(super) fn common_modular_next_multiple_of_assign_uint<U>(&mut self, rhs: U, modulo: &Self)
    where U: TraitsBigUInt<U>
    {
        if *self >= *modulo
            { self.wrapping_rem_assign(modulo); }
    ...
    pub(super) fn common_modular_next_multiple_of_assign(&mut self, rhs: &Self, modulo: &Self)
    {
        self.wrapping_rem_assign(modulo);
        let mrhs;
        if rhs.ge(modulo)
...
where T: TraitsBigUInt<T>
{
    ...
    where U: TraitsBigUInt<U>
    {
        biguint_modular_calc_assign!(self, Self::common_modular_next_multiple_of_assign_uint, rhs, modulo);
    }
    ...
    {
        biguint_modular_calc_assign!(self, Self::common_modular_next_multiple_of_assign, rhs, modulo);
    }
}
